...
export class GistServiceService {
  updateLibrary(username: string) {
    return this.http.get(environment.gistService + "/refresh/" + username, {
      responseType: "text",
    });
  }

  getAllSnippets(username: string): Observable<Snippet[]> {
    return this.http.get<Snippet[]>(
      environment.gistService + "/snippets/" + username + "/all"
    );
  }
  ...
  getTagSnippets(id: string): Observable<Snippet[]> {
    return this.http.get<Snippet[]>(
      environment.gistService + "/snippets/tag/" + id
    );
  }
